...
def MGD(Func,x, erro,k_max):
    ...
    while A:
        ...
        # lam = co.cond_gra_simp(matrix, np.zeros(len(matrix)))
        if ite > 1:
            lam_old = lam
        # print(G,g,x,l)
        lam = co.cond_gra_simp_prox1(G, g, x, l, lam)

        y = co.proj_unit_simplex_median(x - np.dot(G.T, lam))

        v = y - x
        psi_x = np.dot(G, v)
        # t_x = np.max((np.dot(G,v) + (Func.g(y) - g)) / alpha) + 0.5 * np.dot(v,v)
        print("ite", ite, "v",np.linalg.norm(x - y))
        # print("value", np.linalg.norm(v))
        # print("v",np.linalg.norm(v),lam)
        # print(np.dot(lam_old,Func.Val(x)),np.dot(lam,Func.Val(x)))

        if np.linalg.norm(x - y) < 1 * erro or np.linalg.norm(
                x - y) < 1 * 1e-7:  # 防止proximal在大的光滑参数不稳定
            g = np.ones(2)
            G = Func.Gra(x)
            l1 = 1 / n * np.ones(len(g))

            lam1 = co.cond_gra_simp_prox1(G, g, x, l1, lam)
            # print("lam1", lam1)
        ...
        while B:
            ...
            if Func.Ifindomain(x + t * v):
                if k > 20 or t < 1e-6:
                    ...
                    k += 1
                    # print("OK and t is:{}".format(t))
                    # print(np.dot(lam, Func.Val(x + t * v)), np.dot(lam, Func.Val(x)))
                    stepsize.append(t)
                    # print(stepsize)

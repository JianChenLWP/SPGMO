import numpy as np
import copy
################################ L_{1} composite problem ################################################################################
def soft(l,x):   #  prox_{l\|\cdot\|_{1}}(x)
...
#     lb = np.min(y) - 2 / n
#     ub = np.max(y)
#     while np.sum(np.maximum(y - lb, 0)) - np.sum(np.maximum(y- ub, 0)) > 1e-9:
#         a = (lb + ub) / 2
#         if np.sum(np.maximum(y - a, 0)) - 1 > 0:
#             lb = a
#         else:
#             ub = a
#         print("ccc")
#     return np.maximum(y - lb, 0)
...
def proj_unit_simplex(y):
    ...
    # suffix_sum[k] = y_sorted[k] + y_sorted[k+1] + ... + y_sorted[n-1]
    suffix_sum = np.cumsum(y_sorted[::-1])[::-1]

    t_hat = None

    # Algorithm uses 1-based index i = n-1, ..., 1
    # Here i means the same 1-based index as in the paper
    for i in range(n - 1, 0, -1):
        # Compute:
        # t_i = (sum_{j=i+1}^n y_(j) - 1) / (n - i)
        #
        # In Python:
        # y_(i+1) corresponds to y_sorted[i]
        # y_(i) corresponds to y_sorted[i-1]
        t_i = (suffix_sum[i] - 1.0) / (n - i)
    ...
    # Step 4: if no such i is found, all components are active
    if t_hat is None:
        t_hat = (np.sum(y_sorted) - 1.0) / n

    # Step 5: return x = (y - t_hat)_+
...
def proj_unit_simplex_median(y, z=1.0, verbose=False):
    """
...
def w(JF, fy, Fx, yk, L, l,  prox, x):   #Frank-Wolfe算法中的目标函数  g = 1/n*\|\cdot\|_{1}
    ...
    Moreau = np.dot(x,l) / L * np.linalg.norm(prox, ord=1) + 0.5 * np.dot(s,s)
...
def cond_gra_simp_prox(JF, fy, Fx, yk, L, l, lam):  #proximal的away-step条件梯度法,  l为正则项系数，只考虑LASSO问题
    ...
    prox_old = soft(np.dot(x_old, l) / L, yk - np.dot(JF.T, x_old) / L)  # prox_{\lambda g}(x_k - \sum\lambda\nabla f(x))
    grad_old = Fx - fy - np.dot(JF, prox_old - yk) - l * np.linalg.norm(prox_old, ord=1)
    # print("a")
    while Bull:
        # print("b")
        Bull = False
        prox = soft(np.dot(x, l) / L, yk - np.dot(JF.T, x) / L)  # prox_{\lambda g}(x_k - \sum\lambda\nabla f(x))
        grad = Fx - fy - np.dot(JF, prox - yk) - l * np.linalg.norm(prox, ord=1)
        ...
        d = proj_unit_simplex(x - 1 / alpha * grad) - x
        if P:
            print("bbb")

        if alpha * np.linalg.norm(d) > 1e-9 and np.linalg.norm(d) > 1e-11:
            Bull = True
            t = 1
            Bull2 = True
            iite = 0
            while Bull2:
                ...
                iite += 1
                Bull2 = False
                prox_new = soft(np.dot(x + t * d, l) / L, yk - np.dot(JF.T, x + t * d) / L)
...
def w1(JF, g, xk, l,  prox, x):   #Frank-Wolfe算法中的目标函数  g = 1/n*\|\cdot\|_{1}
    ...
    Moreau = np.dot(x,l) * np.linalg.norm(prox, ord=1) + 0.5 * np.dot(s,s)
...
# print or not
    ...
    n = len(x)
    x_old = np.random.dirichlet(np.ones(n))
    prox_old = soft(np.dot(x_old, l), xk - np.dot(JF.T, x_old))  # prox_{\lambda g}(x_k - \sum\lambda\nabla f(x))
    grad_old = g - np.dot(JF, prox_old - xk) - l * np.linalg.norm(prox_old, ord=1)
    while Bull:
        Bull = False
        prox = soft(np.dot(x, l), xk - np.dot(JF.T, x))  # prox_{\lambda g}(x_k - \sum\lambda\nabla f(x))
        grad = g - np.dot(JF, prox - xk) - l * np.linalg.norm(prox, ord=1)
        ...
        if alpha * np.linalg.norm(d) > 1e-9 and np.linalg.norm(d) > 1e-11:
            ...
            while Bull2:
                iite += 1
                Bull2 = False
                prox_new = soft(np.dot(x + t * d, l), xk - np.dot(JF.T, x + t * d))
